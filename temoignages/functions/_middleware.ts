 ...
 *   TI_ADMIN_BASIC_USER
 *   TI_ADMIN_BASIC_PASS
 *   TI_TEMOIGNAGES_API_KEY   (optional, for scripted export)
 *
...
function unauthorized(): Response {
  return new Response('Unauthorized', {
    status: 401,
    headers: {
      'WWW-Authenticate': 'Basic realm="TeachInspire Temoignages", charset="UTF-8"',
...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  const apiKey = context.env.TI_TEMOIGNAGES_API_KEY ?? '';
